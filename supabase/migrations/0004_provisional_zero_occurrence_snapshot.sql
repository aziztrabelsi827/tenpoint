-- ============================================================================
-- TenPoint — 0004: provisional zero-progress occurrences no longer freeze config
-- ----------------------------------------------------------------------------
-- 0002 introduced write-once snapshot semantics for habit_logs: once a row
-- existed (even at count = 0), a later re-check kept scoring against the day's
-- ORIGINAL snapshot. That froze the reward a habit earned the first time an
-- occurrence was toggled and cleared, so editing the habit's weight/repetitions
-- afterwards never applied to that day again — a user changing `max points`
-- from 0.5 to 1.0 before completing expected the new reward but the RPC stored
-- points computed from the stale 0.5 snapshot.
--
-- Corrected rule (mirrors src/lib/scoring.ts resolveHabitSnapshot):
--
--   A stored snapshot is authoritative ONLY when the day holds REAL recorded
--   progress (count > 0). A zero-progress row is PROVISIONAL — it marks
--   participation but freezes nothing. When real progress is first recorded,
--   the habit's CURRENT configuration becomes the day's snapshot and the
--   stale provisional values are overwritten.
--
-- SECURITY model is unchanged: SECURITY INVOKER, row-locked serialisation and
-- the same grants, so this stays under the user's RLS exactly like 0002.
-- No tables, columns, constraints, indexes or RLS policies are altered — this
-- only re-creates the function body.
-- ============================================================================
